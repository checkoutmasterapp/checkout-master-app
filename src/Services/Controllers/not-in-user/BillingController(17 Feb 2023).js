...
module.exports.billing_details = async (req, res, next) => {
	...
	if (req.method === "POST") {
		try {
			...
			console.log("billing_details user_subscription_billings --------")
			...
			// Stripe Invoice Paid 
			const payInvoice = await stripe.invoices.retrieve(stripe_subscription.latest_invoice);

			return res.json({
				status: true,
				message: "Billing create successfully",
				redirect_url: payInvoice?.hosted_invoice_url,
			});
		} catch (error) {
