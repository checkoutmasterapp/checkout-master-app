...
module.exports.billing_details = async (req, res, next) => {
    ...
    if (req.method === "POST") {
        try {
            ...
            let stripe_card_token, stripe_card_id, stripe_card_last4, exp_month, exp_year, stripe_card_brand;
            let stripe_charge_response = {};

            let card_verification = false;
            if (request_body?.selected_card && request_body?.selected_card !== "new_card") {
                stripe_card_id = request_body?.selected_card;
                ...
                await models.UserSubscriptionCardDetails.update({
                    is_default: false
                }, {
                    where: {
                ...
                if (request_body.card_number && request_body.expiry_date && request_body.card_cvv) {
                    ...
                    let stripe_card_source = await stripe.customers.createSource(stripe_customer_id, {
                ...
                //////////////////////////////////// An authorisation of 1$ will be made to validate your credit card
                if (stripe_card_token) {
                    let authorisation_amount = 1;
                    stripe_charge_response = await stripe.charges.create({
                        currency: "usd",
                        amount: parseFloat(authorisation_amount) * 100,
                        customer: stripe_customer_id,
                    });
                }

                card_verification = stripe_charge_response?.status === "succeeded" ? true : false;
                await models.UserSubscriptionCardDetails.update({
                    is_default: false
                }, {
                    where: {
            ...
            if (card_verification === true) {
                ...
                let current_user_subscription = await models.UserSubscriptions.findOne({
                    where: { store_id: request_body?.store_id },
                });
                ...
                if (stripe_card_token?.card?.id) {
                    user_subscription_card_detail = await models.UserSubscriptionCardDetails.create({
                        user_id: auth_user?.id,
                        store_id: request_body?.store_id,
                        stripe_customer_id: stripe_customer_id,
                        stripe_card_id: stripe_card_id,
                ...
                let stripe_invoice_number = `${subscription_package?.id}-${auth_user?.id}-${today}`;
                ...
                current_user_subscription.status = true;
                await current_user_subscription.save();

                await CheckStoreBillingAndRenew(request_body?.store_id);
            }
