...
module.exports.StripeWebhook = async (req, res) => {
    ...
    try {
        ...
        let package_detail, request_body;
        ...
        switch (event.type) {
            ...
            case "customer.subscription.created":
                ...
                request_body = temp_subscription?.request_body;
            ...
            case "customer.subscription.updated":
                ...
                ///////////////////////////// Get Subsciption Charge Details Start
                const subscription_update_invoice = await stripe.invoices.retrieve(subscription_update.latest_invoice);

                var current_period_end = new Date(subscription_update.current_period_end * 1000);
                var current_period_start = new Date(subscription_update.current_period_start * 1000);
                console.log("subscription_update current_period_end -------------", current_period_end);
                console.log("subscription_update current_period_start -------------", current_period_start);

                ///////////////////////////// Get Subscription Package Detail
                package_detail = await models.SubscriptionPackage.findOne({
                    where: {
                        stript_object_id: subscription_update?.items?.data[0]?.plan?.id
                    }
                });

                ///////////////////////////// Get Current User Details
                let current_user = await models.Users.findOne({
                    where: { stripe_customer_id: subscription_update?.customer }
                });
                ...
                let pending_subscription_billing = await models.UserSubscriptionBillings.findOne({
                    where: {
                        status: "Pending",
                        stripe_subscription_id: subscription_update?.id
                    },
                });
                if (pending_subscription_billing) {

                    let subscription_billing_status = "Pending";
                    if (subscription_update?.status === "active") {
                        subscription_billing_status = "Active";
                    } else if (subscription_update?.status === "incomplete") {
                        subscription_billing_status = "incomplete";
                    }

                    pending_subscription_billing.user_id = current_user?.id;
                    pending_subscription_billing.user_subscription_id = current_user_subscription?.id;
                    pending_subscription_billing.subscription_package_id = package_detail?.id;
                    pending_subscription_billing.billing_cycle = package_detail?.billing_cycle;
                    pending_subscription_billing.price = subscription_update_invoice.total / 100;
                    pending_subscription_billing.start_date = current_period_start;
                    pending_subscription_billing.end_date = current_period_end;
                    pending_subscription_billing.status = subscription_billing_status;

                    pending_subscription_billing.stripe_subscription_id = subscription_update_invoice.subscription;
                    pending_subscription_billing.stripe_customer_id = subscription_update_invoice?.customer;
                    pending_subscription_billing.stripe_invoice_id = subscription_update_invoice.id;
                    pending_subscription_billing.stripe_invoice_number = subscription_update_invoice?.number;
                    pending_subscription_billing.stripe_invoice_pdf = subscription_update_invoice?.invoice_pdf;

                    pending_subscription_billing.save();

                    if (subscription_update?.status === "active") {
                        //////////////////////////////////// Update User subscription Data
                        current_user_subscription.status = true;
                        current_user_subscription.is_expired = false;
                        current_user_subscription.end_date = current_period_end;
                        current_user_subscription.subscription_package_id = package_detail?.id;
                        current_user_subscription.billing_id = pending_subscription_billing?.id;
                        current_user_subscription.billing_cycle = package_detail?.billing_cycle;
                        current_user_subscription.save();
                    }
                } else {
                    let subscription_billing_status = "Pending";
                    ...
                    if (subscription_update?.status === "incomplete") {
                        subscription_billing_status = "incomplete";
                    }
                    ...
                    let subscription_billings = {
                        ...
                        subscription_package_id: package_detail?.id,
                        billing_cycle: package_detail?.billing_cycle,
                        price: subscription_update_invoice.total / 100,
                        start_date: current_period_start,
                        end_date: current_period_end,
                        status: subscription_billing_status,

                        stripe_subscription_id: subscription_update_invoice.subscription,
                        stripe_customer_id: subscription_update_invoice?.customer,
                        stripe_invoice_id: subscription_update_invoice.id,
                        stripe_invoice_number: subscription_update_invoice?.number,
                        stripe_invoice_pdf: subscription_update_invoice?.invoice_pdf,
