...
module.exports.ApplePayPaymentCreate = async (request_body, callback) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let applePaymentToken = JSON.parse(request_body?.applePaymentToken);
            ...
            let temp_checkout = await models.TempCheckout.create({
                ...
                payment_id: applePaymentToken?.transactionIdentifier,
                payment_response: applePaymentToken,
            });

            let shopify_response = await ApplePayPaymentSuccess({ payment_id: applePaymentToken?.transactionIdentifier });
            callback(null, shopify_response);
        } catch (error) {
            console.error("ApplePayPaymentCreate error -----------------", error);
            reject({
                status: false,
...
const ApplePayPaymentSuccess = async (request_body) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (temp_checkout) {
                ...
                if (apply_discount) {
                    ...
                    console.log("ApplePayPaymentSuccess discount_codes -------", discount_codes);
                ...
                console.log("ApplePayPaymentSuccess order shopify_order_body ------------", shopify_order_body);
                ...
                // console.log("ApplePayPaymentSuccess order shopify_order ------------", shopify_order);
            ...
            console.error("ApplePayPaymentSuccess error -----------------", error);
            reject({
                status: false,
                message: error?.message ? error.message : "Something went wrong. Please try again.",
            });
