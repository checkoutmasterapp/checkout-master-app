!(function () {
    const action = {
        config: {
            ...
            defaultAppBaseUrl: "https://app.checkout-master.com",
        ...
        functions: {
            ...
            getAPPCheckoutURL: async () => {
                ...
                console.log("getAPPCheckoutURL url ------------", url);
                ...
                return await fetch(url, {
                    ...
                    console.log("getAPPCheckoutURL response ------------", response);
                    return response.status === true ? response.checkout_url : checkUrl;
                });
            },
            processCheckout: async () => {
                await action.functions.checkoutCheck().then((res) => {
                    return res.status ? res.json() : { failure: true };
                }).then(function (res) {
                    return !res.failure ? localStorage.setItem("master_x_s", res?.store_detail?.checkout_uuid) : true;
                }).then(async (result) => {
                    await action.cartApi.syncCart().then(async (response) => {
                        if (action.variables.isCheckoutProcessing === false && response === true) {
                            ...
                            const checkout_id = localStorage.getItem("master_x_s");
                            const checkout_url = await action.functions.getAPPCheckoutURL();

                            if (checkout_url && checkout_id && shop_id) {
                                action.variables.isCheckoutProcessing = false;
                                const run = `${checkout_url}/${shop_id}/checkout/${checkout_id}`;
                                window.location = run;
                            } else {
                                action.variables.isCheckoutProcessing = false;
            ...
            addHandlers: () => {
                ...
                [...checkoutForm].forEach((element) => {
                    ...
                    //         console.log("checkoutForm, ", element);
            ...
            addDynamicButtonHandlers: () => {
                ...
                // console.log("addDynamicButtonHandlers");
            },
            preventCheckoutFormSubmission: () => {
                // let cartDrawer = document.querySelector(".cart_drawer");
                // if(cartDrawer) {
                // [...action.dom.getCheckoutForms()].forEach((form) => {
                // console.log("form", form);
                document.body.addEventListener("click", function (e) {
                    let element = e.target;
                    console.log("clicked", element);
                    let checkoutButton = element.closest("button[name=checkout]")
                    if (checkoutButton) {
            ...
            init: async () => {
                ...
                await action.functions.CheckStoreSubscription();

                if (action.config.is_expired === false) {
    ...
    action.functions.init(),
        action.functions.addStyle(),
        action.functions.loadCheckoutDomain(),
        action.functions.loadLang();
})();