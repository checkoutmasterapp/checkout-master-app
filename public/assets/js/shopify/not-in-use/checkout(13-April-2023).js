...
jQuery(document).ready(function () {
    ...
    let sub_total = jQuery('input[name="subtotal"]').val()
    ...
    jQuery(document).on("change", 'select[name="shipping_country"]', function (event) {
                    ...
                    shipping_detail_html += `
						<label class="main-full-width">
							<div class="main-inner-free">
								...
								<span>${shipping_option ?
                            shipping_option.shipping_rate_price == 0 ? "Free" :
                                `${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}`
                                + parseFloat(shippingRate).toFixed(2) : "n/a"}</span>
							</div>
						</label>
					`;
                }
        ...
        if (shipping_detail_html) {
			...
			`);
            jQuery('.free-shipping').html(`<span>Delivery</span><span>--</span>`);
            // let total_price = parseFloat(totalPrice);

            if (discount_type == "free_shipping") {
                jQuery('#discount_web').hide();
            }
            if (discount_type == "percentage") {
                if (entire_order) {
                    total_price = parseFloat(totalPrice) - parseFloat(sub_total * discount_value / 100);
                    jQuery("#discount_price").html(`-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${(sub_total * discount_value / 100).toFixed(2)}`);
                } else if (discount_applicable_items && include) {
                    total_price = parseFloat(totalPrice) - parseFloat(discount_value);
                    jQuery("#discount_price").html(`-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${discount_value}`);
                } else {
                ...
                if (discount_applicable_items && include && discount_buy_items) {
                    fixed_amount(0);
                } else {
                    let total_price = parseFloat(totalPrice);
                    jQuery('#discount_web').hide();
            ...
            if (jQuery('input[name="total_price"]').val() == 0) {
                jQuery(".credit_card_section").hide();
                // jQuery('.apply_button').attr("disabled", true);
                jQuery('.radio-purches-btn').prop("checked", true);
                jQuery('.payment_shipping').prop("checked", false);
                jQuery('.payment_shipping').attr("disabled", true);

                jQuery(".payment_method_free_purchase").show();
                jQuery(".payment_method_free_purchase_input").prop("checked", true);
            } else {
                jQuery(".credit_card_section").show();
                // jQuery('.apply_button').attr("disabled", true);
                jQuery('.radio-purches-btn').prop("checked", false);
                jQuery('.payment_shipping').prop("checked", true);
                jQuery('.payment_shipping').attr("disabled", false);
    ...
    jQuery(document).on("click change", 'input[name="shipping_rate"]', function (event) {
        ...
        let discount_code = jQuery('#discount_code').val();

        let discount_code_type = jQuery('#discount_price').attr('data-type');

        if (discount_type == "free_shipping") {
            let total_price = parseFloat(totalPrice);
            if (shipping_rate == 0 || parseFloat(shipping_rate) > parseFloat(exclude_shipping_amount) && parseFloat(exclude_shipping_amount) !== 0) {
                jQuery('#discount_web').hide();
                jQuery("#discount_code_web").hide();
                total_price = parseFloat(totalPrice) + parseFloat(shipping_rate);
            } else {
                jQuery("#discount_code_web").show();
                jQuery('#discount_web').show();
                jQuery("#discount_price").html(`
                ${shipping_rate == 0 ?
                        "Free" : `-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + shipping_rate.toFixed(2)}
                `);
            }
            total_price_div(total_price);

        } else if (discount_type == "percentage") {
            console.log(discount_applicable_items, '---------discount_applicable_items', include)
            if (entire_order) {
                total_price = parseFloat(totalPrice) + parseFloat(shipping_rate) - parseFloat(sub_total * discount_value / 100);
                jQuery("#discount_price").html(`-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${(sub_total * discount_value / 100).toFixed(2)}`);
            } else if (discount_applicable_items && include) {
                total_price = parseFloat(totalPrice) + parseFloat(shipping_rate) - parseFloat(discount_value);
                jQuery("#discount_price").html(`-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${discount_value}`);
            } else {
                total_price = parseFloat(totalPrice) + parseFloat(shipping_rate);
                jQuery('#discount_web').hide();
        ...
        jQuery(".free-shipping").html(`
			${shipping_rate_name} <span class="shipping_rate">
            ${shipping_rate == 0 ?
                "Free" : `${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + shipping_rate.toFixed(2)}</span>
    ...
    function fixed_amount(shipping_rate) {
        if (shipping_options?.length > 0) {
            if (parseFloat(discount_value) > parseFloat(sub_total)) {
                let total_price = parseFloat(totalPrice) + parseFloat(shipping_rate) - parseFloat(sub_total);
                jQuery("#discount_price").html(`-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${sub_total}`);

                total_price_div(total_price);
            } else {
                let total_price = parseFloat(totalPrice) + parseFloat(shipping_rate) - parseFloat(discount_value);
                jQuery("#discount_price").html(`-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${discount_value}`);
                total_price_div(total_price);
            }
        }
            ...
            if (parseFloat(discount_value) > parseFloat(sub_total)) {
                let total_price = parseFloat(totalPrice) - parseFloat(sub_total);
                jQuery("#discount_price").html(`-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${sub_total}`);
                total_price_div(total_price);
            } else {
                let total_price = parseFloat(totalPrice) - parseFloat(discount_value);
                jQuery("#discount_price").html(`-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${discount_value}`);
                total_price_div(total_price);
            }
        }
    ...
    function total_price_div(total_price) {
        jQuery(".total-span").html(`${total_price == 0 ?
            "Free" : `${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + total_price.toFixed(2)}
                <input type="hidden" name="subtotal" value="${totalPrice.toFixed(2)}" />
                <input type="hidden" name="total_price" value="${total_price.toFixed(2)}" />
            `);
        jQuery("#button_price").html(`${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}${total_price.toFixed(2)}`);
    }

    /****************Check discount code ************/
    jQuery(document).on("click", '.apply_button', function (event) {
        let discount_code = jQuery('#discount_code').val();
        console.log("discount-------------", discount_code)
        var shipping_rate = jQuery('input[name="shipping_rate_amount"]').val();
        if (discount_code) {
            jQuery.ajax({
                type: "GET",
                dataType: "json",
                url: `${ajax_url}/check-discount-code`,
                data: { discount_code: discount_code, store_id: store_id, checkout_id: checkout_id },
                success: function (res) {
                    if (res?.status) {
                        jQuery('.apply_div').html(`
                            <button class="cancel_discount" type="button" id="btn-preview" style="border-color:#1a1a1a;color:#1a1a1a">X</button>
                        `)
                        var html = `<span>Discount<strong id="discount_title"style="color:#6f6601"><i class="bi bi-tag ms-2"></i>${discount_code}</strong></span>`
                        var discount_value = res?.discount_value_price.split('-')[1];

                        let discount_web = jQuery("#discount_web").html();

                        if (res?.discount_value_type == "percentage" && res?.target_type == "line_item") {
                            total_price = parseFloat(totalPrice) + parseFloat(shipping_rate) - parseFloat(sub_total * discount_value / 100);
                            html += `<span id="discount_price">-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${(sub_total * discount_value / 100).toFixed(2)}</span>`
                            jQuery("#discount_web").show();
                            if (!discount_web) {
                                jQuery("#discount_code_web").removeClass('displaynone_important');
                                jQuery("#discount_code_web").html(html);
                            } else {
                                jQuery("#discount_web").html(html);
                            }

                        } else if (res?.discount_value_type == "fixed_amount" && res?.target_type == "line_item") {
                            total_price = parseFloat(totalPrice) + parseFloat(shipping_rate) - parseFloat(discount_value);
                            if (parseFloat(discount_value) > parseFloat(sub_total)) {
                                total_price = parseFloat(totalPrice) + parseFloat(shipping_rate) - parseFloat(sub_total);
                                html += `<span id="discount_price" data-type="fixed_amount" data-discount= "${discount_value}">-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${sub_total}`;
                            } else {
                                html += `<span id="discount_price" data-type="fixed_amount" data-discount= "${discount_value}">-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${discount_value}`;
                            }
                        ...
                        } else if (res?.discount_value_type == "percentage" && res?.target_type == "shipping_line") {
                            total_price = parseFloat(totalPrice)
                            html += `<span id="discount_price" data-type="free_shipping">-${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}` + `${shipping_rate}</span>`
                            jQuery("#discount_web").show();
                            if (!discount_web) {
                                jQuery("#discount_code_web").removeClass('displaynone_important');
                                jQuery("#discount_code_web").html(html);
                        ...
                        ${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}${total_price.toFixed(2)}
                            <input type="hidden" name="subtotal" value="${totalPrice.toFixed(2)}" />
                            <input type="hidden" name="total_price" value="${total_price.toFixed(2)}" />
                        `);
                        jQuery("#button_price").html(`${customize_checkout.money_format ? customize_checkout.money_format.substring(0, 1) : "$"}${total_price.toFixed(2)}`);
                        ...
                        jQuery('.discount_error').html("Make sure that the code you have applied is correct, not used before or expired.")
                    }
                },
                error: function (err) {
    ...
    /****************Cancel apply discount code *****************/
    jQuery(document).on("click", '.cancel_discount', function (event) {
        console.log("clicked--------")
        jQuery('.apply_div').html(`
            <button class="apply_button" type="button" id="btn-preview" style="border-color:#1a1a1a;color:#1a1a1a">Apply</button>
        `)
        jQuery('.input_discount_code').html(`<input type="text" name="discount_code" placeholder="Discount code" id="discount_code" style="border-color: rgb(199, 199, 199);">`)
    })
...
async function paymentGateway(select_option) {
    ...
    switch (select_option) {
        case "Stripe":
            StripePayment(product_details, checkout_detail, discount_value, discount_type, discount_id);
            break;
        case "PayPal":
            PayPalPayment(product_details, checkout_detail, discount_value, discount_type, discount_id);
            break;
        case "Payout Master":
            PayoutMasterPayment(product_details, checkout_detail, discount_value, discount_type, discount_id);
            break;
        case "Checkout.com":
            CheckoutPayment(product_details, checkout_detail, discount_value, discount_type, discount_id);
            break;
        case "Revolut":
            RevolutPayment(product_details, checkout_detail, discount_value, discount_type, discount_id);
            break;
        case "Apple pay":
            ApplePayPayment(product_details, checkout_detail);
            break;
        case "cash_on_delivery":
            CashOnDelivery();
            break;
        case "free_payment":
            FreePayment(product_details, checkout_detail, discount_value, discount_type, discount_id);
            break;
        default:
            break;
    }
...
async function FreePayment(product_details, checkout_detail) {
    var discount_title = $('#discount_title').text();
    var total_price = jQuery('input[name="total_price"]').val();
    var shipping_rate_amount = jQuery('input[name="shipping_rate_amount"]').val();

    let customer_detail = CustomerDetail(checkout_detail, "Stripe");

    if (discount_type == "percentage") {
        if (entire_order) {
            discount_value = jQuery('input[name="subtotal"]').val() * discount_value / 100
        }
    }

    if (discount_type == "free_shipping") {
        console.log(exclude_shipping_amount, shipping_rate_amount)
        if (parseFloat(shipping_rate_amount) > parseFloat(exclude_shipping_amount)) {
            discount_value = null;
        } else {
            discount_value = shipping_rate_amount
        }
    }

    let subtotal = jQuery('input[name="subtotal"]').val();
    if (discount_type == "fixed_amount") {
        if (parseFloat(discount_value) > parseFloat(subtotal)) {
...
async function PayPalPayment(product_details, checkout_detail, upsellTriggerData) {
    var payment_url = "";
    var productData = [];
...
async function PayoutMasterPayment(product_details, checkout_detail) {
    let customer_detail = CustomerDetail(checkout_detail, "Revolut");
    console.log("PayoutMasterPayment customer_detail ---------------", customer_detail);
...
async function CheckoutPayment(product_details, checkout_detail) {
    var discount_title = $('#discount_title').text();
    var shipping_rate_amount = jQuery('input[name="shipping_rate_amount"]').val();
    if (discount_type == "percentage") {
        if (entire_order) {
...
async function RevolutPayment(product_details, checkout_detail, upsellTriggerData, upsellData, discount_value, discount_type) {
    let customer_detail = CustomerDetail(checkout_detail, "Revolut");
    console.log("RevolutPayment customer_detail ---------------", customer_detail);
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            console.log("RevolutPayment pay response -------------", response);
            if (response.status) {
                // window.location.href = response?.response?.checkout_url;
                let payment_type = (PAYMENT_MODE === "live") ? "prod" : "sandbox";
                console.log("RevolutPayment pay payment_type -------------", payment_type);
                ...
                RevolutCheckout(response.public_id, payment_type).then(function (RC) {
                    RC.payWithPopup({
                        ...
                        onSuccess() {
                            RevolutPaymentSuccess(response.revolut_order, upsellTriggerData, upsellData);
                        },
...
async function RevolutPaymentSuccess(revolut_order, upsellTriggerData, upsellData) {
    console.log(upsellTriggerData, '----------upsellTriggerData')
    jQuery.ajax({
        type: "POST",
        dataType: "json",
        url: `${ajax_url}/pay`,
...
async function ApplePayPayment(product_details, checkout_detail, upsellTriggerData, upsellData) {
...
async function ApplePayPaymentSuccess(applePaymentToken) {
    console.log("##############################################");
    console.log("ApplePayPaymentSuccess applePaymentToken ----------", applePaymentToken);
